...
\begin{equation}
X_i^L=X_i+0.03\max(0.05,1-N_{eval}/B_g)L_i(1.5),
\end{equation}
where $L_i$ follows the Mantegna construction~\cite{Mantegna1994}. The original CBO candidate and the additional L\'{e}vy candidate are separately counted and greedily accepted.
...
The CEC2017 convergence checkpoints are retained as diagnostic data in the reproducibility package. The primary CEC2017 evidence reported in this manuscript is the complete 29-function final-rank analysis, because final error ranks and paired tests provide a more stable basis for comparison than selected convergence curves.
...
\begin{table*}[ht]
\centering
\caption{Mean paired relative improvement of LSCBO over Greedy-LPT. Parentheses give paired wins out of 30 seeds; all remaining cases are ties, with no component losses.}\label{tab:main_improvement}
\resizebox{\textwidth}{!}{%
\begin{tabular}{l c c c c c}
\toprule
...
The no-regression rule is visible in Table~\ref{tab:main_improvement}: all mean changes are nonnegative and there are no component losses in any block. The practical effect is concentrated in LBR. Objective reductions are consistent but small and decline with task count.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_main_multicost_improvement.png}
\caption{Mean component-wise improvement of LSCBO over Greedy-LPT. The zero line denotes equality; positive values favor LSCBO.}\label{fig:multicost_profile}
\end{figure*}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/fig_main_paired_improvement.png}
\caption{Paired objective improvement over Greedy-LPT, with 95\% confidence intervals over 30 seeds. The vertical scale is in percent and should be interpreted as a small refinement.}\label{fig:paired_improvement}
\end{figure}
...
\bibitem{Qian2025}
Qian Z, Zhang Y, Pu D, Xie G, Pu D, Ye M (2025) A new hybrid improved Kepler optimization algorithm based on multi-strategy fusion and its applications. Mathematics 13(3):405. \url{https://doi.org/10.3390/math13030405}
