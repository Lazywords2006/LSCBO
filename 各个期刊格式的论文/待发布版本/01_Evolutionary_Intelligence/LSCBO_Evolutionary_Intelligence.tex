...
The no-regression rule is visible in Table~\ref{tab:main_improvement}: all mean changes are nonnegative and there are no component losses in any block. The practical effect is concentrated in LBR. Objective reductions are consistent but small and decline with task count. Figure~\ref{fig:cloud_objective_comparison} presents the scalar comparison in a scale-wise multi-algorithm layout: LSCBO with pair reassignment is the only method with a visible objective reduction at every task count, while CBO and PSO remain near zero and GWO, WOA, and AOA coincide with Greedy-LPT. Figure~\ref{fig:cloud_component_profiles} separates the four reported components and shows that the visible improvement is concentrated in LBR; makespan, energy, and cost changes are much smaller.

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_cloud_objective_comparison.png}
\caption{Scale-wise cloud objective comparison. Bars show paired mean objective improvement over Greedy-LPT across 30 seeds. Positive values indicate a lower reference-normalized scalar objective. GWO, WOA, and AOA are grouped with Greedy-LPT because their formal traces coincide with the deterministic warm-start schedule.}\label{fig:cloud_objective_comparison}
\end{figure*}

\begin{figure*}[htbp]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig_cloud_component_profiles.png}
\caption{Component-level cloud scheduling comparison across task scales. Positive values indicate component-wise improvement over Greedy-LPT. The figure separates makespan, energy, execution cost, and LBR to show that the main visible change comes from load-balance refinement, while physical-cost changes remain small. GWO, WOA, and AOA are grouped with Greedy-LPT because their formal traces are identical under the reported protocol.}\label{fig:cloud_component_profiles}
\end{figure*}
