...
/**
 * 5算法CEC2017基准测试完整对比实验
 *
 * 测试配置：
 * - 5个算法：Random, PSO, WOA, CBO, LSCBO-Fixed
 * - 可选：6个强势函数或30个完整函数集
 * - 30次独立运行（CEC2017标准配置）
 *
 * 实验规模：
 * - 快速验证：5算法 × 3函数 × 10次 = 150次测试（~1-2小时）
 * - 6函数实验：5算法 × 6函数 × 30次 = 900次测试（~6-8小时）
 * - 完整实验：5算法 × 30函数 × 30次 = 4500次测试（~3-4小时）
 *
 * 输出格式：
...
public class SixAlgorithmCEC2017Test {
     ...
     */
    private static final boolean ENABLE_FUNCTION_FILTER = false;  // 测全部30函数（公平，不再挑函数）
     ...
     *
     * （注：函数过滤已禁用 ENABLE_FUNCTION_FILTER=false，默认测全部 30 函数；
     *  此列表仅历史保留，不再用于挑选有利函数。）
     */
    ...
    public static void main(String[] args) {
        System.out.println("\n╔════════════════════════════════════════════════════════════════╗");
        System.out.println("║  CEC2017 Benchmark Test - 5 Algorithms Comparison           ║");
        System.out.println("║  Random | PSO | WOA | CBO | LSCBO-Fixed                    ║");
        System.out.println("╚════════════════════════════════════════════════════════════════╝\n");
    ...
    /**
     * 运行快速验证测试
     * 5算法 × 10函数 × 10次 = 500次测试（~2-3分钟）
     */
    public static void runQuickTest() {
        System.out.println("【快速验证模式 - 10个强势函数】\n");
        System.out.println("测试配置：");
        System.out.println("  - 算法：Random, PSO, WOA, CBO, LSCBO-Fixed");
        System.out.println("  - 函数：" + String.join(", ", SELECTED_FUNCTIONS));
        System.out.println("  - 运行次数：" + QUICK_NUM_RUNS);
        System.out.println("  - 迭代次数：" + MAX_ITERATIONS);
        System.out.println("  - 总测试量：5 × " + SELECTED_FUNCTIONS.size() + " × " + QUICK_NUM_RUNS + " = " + (5 * SELECTED_FUNCTIONS.size() * QUICK_NUM_RUNS) + " 次\n");
    ...
    /**
     * 运行完整实验
     * - 如果启用过滤：5算法 × 10函数 × 30次 = 1500次测试（~3-5小时）
     * - 如果禁用过滤：5算法 × 30函数 × 30次 = 4500次测试（~6-8小时）
     */
    public static void runFullExperiment() {
        ...
        if (ENABLE_FUNCTION_FILTER) {
            System.out.println("  - 函数：" + SELECTED_FUNCTIONS.size() + "个强势函数");
            System.out.println("  - 函数选择：LSCBO/CBO平衡策略 (5+5)");
        } else {
    ...
    private static void printRankingSummary(List<BenchmarkRunner.BenchmarkResult> allResults,
        ...
        System.out.println("\n╔════════════════════════════════════════════════════════════════╗");
        System.out.println("║           5算法整体排名概览                                      ║");
        System.out.println("╚════════════════════════════════════════════════════════════════╝\n");

        // 统计每个算法在所有函数上的平均排名
        String[] algorithmNames = {"Random", "PSO", "WOA", "CBO", "LSCBO-Fixed"};
        double[] avgRanks = new double[5];
