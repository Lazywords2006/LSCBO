 ...
 * - Azure
 * VM定价：https://azure.microsoft.com/en-us/pricing/details/virtual-machines/
 *
...
public class CostCalculator {
    ...
    /**
     * 计算加权适应度函数（论文复现专用）
     * Fitness = w1 * Time + w2 * Load + w3 * Price
     * 
     * 参数设置：
     * P = 8 (价格单价)
     * w1 = 0.5 (时间权重)
    ...
    public static class CostResult {
        ...
        public CostResult(double f, double t, double l, double p) {
            this.fitness = f;
            this.time = t;
            this.load = l;
            this.price = p;
    ...
    public static CostResult calculateWeightedCostDetails(int[] schedule, int M, int N,
            List<Cloudlet> cloudletList, List<Vm> vmList, double w1, double w2, double w3) {
        final double P = 8.0;
        ...
        for (int i = 0; i < M; i++) {
            ...
            double taskLength = cloudletList.get(i).getLength();
            double vmMips = vmList.get(vmIdx).getMips();
            double runtime = taskLength / vmMips;

            vmRuntimes[vmIdx] += runtime;
            totalRuntime += runtime;
        }

        double makespan = 0.0;
        for (double rt : vmRuntimes) {
            if (rt > makespan)
                makespan = rt;
        }

        double meanRuntime = totalRuntime / N;
        double sumSqDiff = 0.0;
        for (double rt : vmRuntimes) {
            sumSqDiff += Math.pow(rt - meanRuntime, 2);
        }
        double load = Math.sqrt(sumSqDiff / N);
        double price = P * totalRuntime;

        // Normalize objectives to [0, ~2] range to prevent Price from dominating
        double normMakespan = makespan / 1000.0;
        double normLoad = load; // Load is already roughly between 0.5 and 1.5
        double normPrice = price / 100000.0;

        double fitness = w1 * normMakespan + w2 * normLoad + w3 * normPrice;

        return new CostResult(fitness, makespan, load, price);
    }

    public static CostResult calculateWeightedCostDetails(int[] schedule, int M, int N,
            List<Cloudlet> cloudletList, List<Vm> vmList) {
        return calculateWeightedCostDetails(schedule, M, N, cloudletList, vmList, 0.5, 0.25, 0.25);
    }

    public static double calculateWeightedFitness(int[] schedule, int M, int N,
            List<Cloudlet> cloudletList, List<Vm> vmList) {
        return calculateWeightedCostDetails(schedule, M, N, cloudletList, vmList).fitness;
    }
}
