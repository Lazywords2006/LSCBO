...
public class CBO_Broker extends DatacenterBrokerSimple {
    ...
    // CBO算法参数 (极速版)
    protected static final int POPULATION_SIZE = 20; // 极度简化
    protected static final int MAX_ITERATIONS = 20; // 极度简化
    ...
    protected void attackingPhase(int M, int N, List<Cloudlet> cloudletList, List<Vm> vmList) {
        for (int i = 0; i < POPULATION_SIZE; i++) {
            ...
            for (int j = 0; j < M; j++) {
                ...
                // Nerfed: 降低向Best收敛的权重 (was 0.5/0.5 -> 0.9/0.1)
