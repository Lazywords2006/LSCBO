...
public class LSCBO_Fixed_Lite implements BenchmarkRunner.BenchmarkOptimizer {
    ...
    // Lévy飞行参数（Dynamic Adaptive Decay - Round 12 - 动态衰减策略）
    private static final double LEVY_LAMBDA = 1.5;        // Lévy分布参数（理论最优值，保持不变）
    private static final double LEVY_ALPHA_COEF = 0.10;   // 🚀 动态步长基数（0.015→0.10,+567%），前期强探索

    // 对数螺旋参数（适中包围 - Round 12）
    private static final double SPIRAL_B = 0.70;           // 🚀 螺旋形状常数（0.95→0.70,-26%），避免过早收敛

    // 自适应惯性权重参数（极致后期收敛 - Round 12）
    private static final double W_MAX = 0.85;             // 最大权重（保持）
    private static final double W_MIN = 0.01;             // 🚀 极致最小权重（0.25→0.01,-96%），后期几乎完全跟随最优

    // 高斯变异参数（动态衰减扰动 - Round 12）
    private static final double SIGMA_MAX = 0.25;         // 最大方差（保持不变）
    private static final double GAUSSIAN_PROB = 0.12;     // 🚀 动态扰动基数（0.03→0.12,+300%），前期跳出局部
