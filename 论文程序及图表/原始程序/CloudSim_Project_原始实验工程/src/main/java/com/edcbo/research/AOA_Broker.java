...
public class AOA_Broker extends DatacenterBrokerSimple {
    ...
    // ==================== 算法参数 ====================
    protected static final int POPULATION_SIZE = 20; // 种群大小 (极速版)
    protected static final int MAX_ITERATIONS = 20; // 最大迭代次数 (极速版)
    ...
    private void runAOA() {
        ...
        int[] initDiscreteSchedule = continuousToDiscrete(bestSolution, N);
        CostCalculator.CostResult initRes = CostCalculator.calculateWeightedCostDetails(initDiscreteSchedule, M, N,
                cloudletList, vmList);
        convergenceRecord.recordIteration(0, initRes.fitness, initRes.time, initRes.load, initRes.price);
        ...
        for (int t = 1; t <= MAX_ITERATIONS; t++) {
            ...
            // 记录详细分量(Cost vs Iterations)
            // AOA logic update best fitness inside loop but only record once per iteration
