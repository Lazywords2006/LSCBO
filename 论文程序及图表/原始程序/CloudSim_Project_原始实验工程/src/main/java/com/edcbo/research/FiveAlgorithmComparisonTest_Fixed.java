...
public class FiveAlgorithmComparisonTest_Fixed {
    ...
    // 实验配置（快速验证模式 - 2×5×5=50实验）
    private static final int[] TASK_SCALES = { 100, 200 }; // 缩减规模以快速获取结果
    private static final long[] SEEDS = { 42, 123, 456, 789, 1024 };
    private static final String[] ALGORITHM_NAMES = { "LSCBO-Fixed", "CBO", "PSO", "HHO", "AOA" };
    ...
    public static void main(String[] args) {
        ...
        int totalExperiments = ALGORITHM_NAMES.length * TASK_SCALES.length * SEEDS.length; // 动态计算
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename, true))) {
            // CSV头
            writer.println(
                    "Algorithm,TaskCount,Seed,CloudSimMakespan,InternalMakespan,LoadBalanceRatio,ExecutionTime_ms");
            writer.flush(); // 立即写入
            ...
            for (int M : TASK_SCALES) {
                for (long seed : SEEDS) {
                    for (String algorithmName : ALGORITHM_NAMES) {
                        ...
                        System.out.println(
                                String.format("     CloudSim Makespan: %.2f (❌ buggy)", result.cloudSimMakespan));
                        System.out.println(
                                String.format("     Internal Makespan: %.2f (✅ correct)", result.internalMakespan));
                        System.out.println(String.format("     Bug放大倍数: %.0fx", bugMultiplier));
                        System.out.println(String.format("     LBR: %.4f | Time: %dms", result.loadBalanceRatio,
        ...
        String outputFile = filename; // 保持兼容性
        ...
        System.out.println("总耗时: " + formatTime((System.currentTimeMillis() - overallStartTime) / 1000));
