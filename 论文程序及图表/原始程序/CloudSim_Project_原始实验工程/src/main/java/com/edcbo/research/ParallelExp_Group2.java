...
/**
 * 并行实验组2：元启发式算法 (LSCBO核心对比)
 * LSCBO, CBO, PSO
 */
public class ParallelExp_Group2 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("=== 并行实验组2: LSCBO核心对比 ===");
        System.out.println("算法: " + Arrays.toString(ALGORITHMS));

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        String rawFile = "results/parallel_group2_" + timestamp + ".csv";
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(rawFile))) {
                ...
                for (int run = 1; run <= RUNS_PER_SCENARIO; run++) {
                    ...
                    for (String algo : ALGORITHMS) {
                        try {
                            ...
                            if (done % 30 == 0)
                                System.out.printf("组2进度: %d/%d (%.1f%%)%n", done, total, 100.0 * done / total);
                        } catch (Exception e) {
            ...
            System.out.println("组2完成! 结果: " + rawFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static DatacenterBrokerSimple createBroker(CloudSimPlus sim, String algo, long seed) {
        switch (algo) {
            case "LSCBO":
                return new LSCBO_Broker_Fixed(sim, seed, "journal");
            case "CBO":
                return new CBO_Broker(sim, seed);
            case "PSO":
                return new PSO_Broker(sim, seed);
            default:
    ...
    private static double getMakespan(DatacenterBrokerSimple broker, String algo) {
        // 始终使用CloudSim仿真的实际完成时间，而不是算法内部的适应度值
        double max = 0;
