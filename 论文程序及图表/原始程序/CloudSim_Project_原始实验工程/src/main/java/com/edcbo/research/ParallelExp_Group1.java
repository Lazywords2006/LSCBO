...
/**
 * 并行实验组1：基线+启发式算法
 * FCFS, RoundRobin, Random, MinMin, MaxMin
 */
public class ParallelExp_Group1 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("=== 并行实验组1: 基线+启发式 ===");
        System.out.println("算法: " + Arrays.toString(ALGORITHMS));

        String timestamp = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        String rawFile = "results/parallel_group1_" + timestamp + ".csv";
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(rawFile))) {
                ...
                for (int run = 1; run <= RUNS_PER_SCENARIO; run++) {
                    ...
                    for (String algo : ALGORITHMS) {
                        try {
                            ...
                            if (done % 50 == 0)
                                System.out.printf("组1进度: %d/%d (%.1f%%)%n", done, total, 100.0 * done / total);
                        } catch (Exception e) {
            ...
            System.out.println("组1完成! 结果: " + rawFile);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private static double[] runExperiment(String algo, int M, long seed) {
        ...
        double makespan = getMakespan(broker);
    ...
    private static DatacenterBrokerSimple createBroker(CloudSimPlus sim, String algo, long seed) {
        switch (algo) {
            ...
            case "RoundRobin":
                return new RoundRobin_Broker(sim, seed);
            case "Random":
                return new Random_Broker(sim, seed);
            case "MinMin":
    ...
    private static double getMakespan(DatacenterBrokerSimple broker) {
        double max = 0;
