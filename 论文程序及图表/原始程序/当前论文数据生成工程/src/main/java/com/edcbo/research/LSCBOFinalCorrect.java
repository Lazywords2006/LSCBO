...
/**
 * LSCBO-LS 最终正确版本（基于 E3 演进路径分析的修正版）
 *
 * 设计：
 *   - CBO 原版三阶段框架（每代三个算子串联，全部跑）
 *   - Phase 1 = Lévy 飞行（替换 tanh 搜索）
 *   - Phase 2 = CBO 旋转矩阵包围（不变）
 *   - Phase 3 = CBO 固定 0.5 权重攻击（去掉动态权重，因为消融证明无效）
 *   - 每 10 代任务迁移局部搜索（memetic）
 *
 * 与"LSCBO-LS-wrong"（带阶段化和动态权重的错版）对比 + 与 8 个 SOTA 对手对比。
 *
 * 输出：results/E4_LSCBOFinal_TIMESTAMP.csv
 */
public class LSCBOFinalCorrect {
    ...
    static final String[] ALGOS = {
        "LSCBO-LS-final",   // 修正后的正确版本
        "LSCBO-LS-wrong",   // 之前带阶段化+动态权重的错版（对照）
        "CBO","GTO","WOA","GWO","HHO","DBO","PSO","AOA"
    };

    public static void main(String[] args) throws Exception {
        String ts = java.time.LocalDateTime.now()
            .format(java.time.format.DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"));
        String out = "results/E4_LSCBOFinal_" + ts + ".csv";
        new java.io.File("results").mkdirs();
        int total = SCALES.length * SEEDS.length * ALGOS.length;
        int done = 0;
        System.out.printf("E4 LSCBO 最终版主对比: %d 算法 x %d 规模 x %d seeds = %d 次%n",
    ...
    static int[] optimize(String algo, double[] vm, double[] cl, int M, long seed) {
        ...
        for (int t=0;t<T;t++) {
            ...
            if (algo.equals("LSCBO-LS-final")) {
                // ★ 修正版：每代三阶段全跑（CBO 原版方式），固定 0.5 权重，无动态/阶段化
                for (int i=0;i<POP;i++) {
                    double[] np = lscboPhase1(pop[i], pop, M, tr, rnd);
                    accept(pop, fit, i, np, vm, cl, M, best);
                    if (fit[i]<bestF) {bestF=fit[i]; best=pop[i].clone();}
                }
                for (int i=0;i<POP;i++) {
                    double[] np = cboPhase2(pop[i], best, M, t, rnd);
                    accept(pop, fit, i, np, vm, cl, M, best);
                    if (fit[i]<bestF) {bestF=fit[i]; best=pop[i].clone();}
                }
                for (int i=0;i<POP;i++) {
                    double[] np = cboPhase3Static(pop[i], best, M);
                    accept(pop, fit, i, np, vm, cl, M, best);
                    if (fit[i]<bestF) {bestF=fit[i]; best=pop[i].clone();}
                }
            } else if (algo.equals("LSCBO-LS-wrong")) {
                // 错版：阶段化 + 动态权重（之前论文里的版本）
                for (int i=0;i<POP;i++) {
                    double[] np;
                    if (tr<1.0/3) np = lscboPhase1(pop[i], pop, M, tr, rnd);
                    else if (tr<2.0/3) np = cboPhase2(pop[i], best, M, t, rnd);
                    else {
                        double w = 0.1+0.8*tr;
                        np = new double[M];
                ...
                for (int i=0;i<POP;i++) {
                    double[] np;
                    switch (algo) {
                        case "CBO": np=cboBaseline(pop[i],pop,best,M,t,tr,rnd); break;
            ...
            // LS: 两个 LSCBO 版本都加
            if ((algo.startsWith("LSCBO-LS")) && t>0 && t%10==0) {
                int[] s = decode(best);
    ...
    static void accept(double[][] pop, double[] fit, int i, double[] np,
                       double[] vm, double[] cl, int M, double[] best) {
        double nf = makespan(decode(np),vm,cl,M);
        if (nf<fit[i]) {pop[i]=np; fit[i]=nf;}
    }

    /** LSCBO Phase 1: Lévy 飞行（替换 CBO 原版的 tanh）。 */
    static double[] lscboPhase1(double[] cur, double[][] pop, int M, double tr, Random rnd) {
        int prey = rnd.nextInt(POP);
        double alpha = 0.05*(1-tr);
        double[] np = new double[M];
        for(int d=0;d<M;d++) np[d]=clamp(cur[d]+alpha*rnd.nextGaussian()*(pop[prey][d]-cur[d]));
        return np;
    }
    /** CBO Phase 2: 旋转矩阵（不变）。 */
    static double[] cboPhase2(double[] cur, double[] best, int M, int t, Random rnd) {
    ...
    /** CBO Phase 3 静态 0.5 攻击（去掉动态权重）。 */
    static double[] cboPhase3Static(double[] cur, double[] best, int M) {
        double[] np = new double[M];
        for(int d=0;d<M;d++) np[d]=clamp(0.5*cur[d]+0.5*best[d]);
        return np;
    }
    /** 标准 CBO baseline (tanh 搜索 + 旋转 + 静态攻击)。 */
    static double[] cboBaseline(double[] cur, double[][] pop, double[] best, int M, int t, double tr, Random rnd) {
        int prey = rnd.nextInt(POP);
        double[] np = new double[M];
        for (int d=0;d<M;d++) {
    ...
    // 其他 SOTA 算子（与 FullMainComparison 相同）
