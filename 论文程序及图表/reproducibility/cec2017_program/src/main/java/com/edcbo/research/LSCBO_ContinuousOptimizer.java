...
/**
 * LF-CBO (Lévy Flight enhanced Colliding Bodies Optimization)
 * Enhanced continuous optimizer for CEC2017 benchmarks
 * 
 * Key improvements:
 * - Larger population for better exploration
 * - Enhanced Lévy flight with adaptive step size
 * - Elite-guided search with random perturbation
 * - Adaptive phase transition based on improvement rate
 */
public class LSCBO_ContinuousOptimizer implements RecordableOptimizer {

    // Optimized algorithm parameters
    private static final int POPULATION_SIZE = 50; // Increased from 30 for better exploration
    private static final int MAX_ITERATIONS = 10000;

    // Enhanced Lévy flight parameters
    private static final double LEVY_LAMBDA = 1.5;
    private static final double LEVY_ALPHA_COEF = 0.1; // Increased from 0.05 for larger steps
    private static final double ELITE_GUIDANCE_PROB = 0.3; // Probability of elite-guided search
    ...
    public LSCBO_ContinuousOptimizer(BenchmarkFunction function, int dimensions, long seed) {
        ...
        this.prevBestFitness = Double.MAX_VALUE;
    ...
    public double optimize() {
        ...
        for (int t = 0; t < MAX_ITERATIONS; t++) {
            ...
            // Track stagnation
            if (t > 0 && bestFitness >= prevBestFitness) {
                stagnationCount++;
            } else {
                stagnationCount = 0;
            }
            ...
            // Adaptive phase thresholds based on stagnation
            double phase1End = 0.25 - 0.1 * (stagnationCount > 50 ? 1 : 0);
            double phase2End = 0.60 - 0.1 * (stagnationCount > 100 ? 1 : 0);

            for (int i = 0; i < POPULATION_SIZE; i++) {
                double[] newPosition = new double[dimensions];

                // Elite guidance with probability
                if (random.nextDouble() < ELITE_GUIDANCE_PROB) {
                    eliteGuidedSearch(i, t, newPosition);
                ...
                if (stagnationCount > 30) {
                    for (int d = 0; d < dimensions; d++) {
                        newPosition[d] += random.nextGaussian() * 0.01 * (upperBound - lowerBound);
                    }
                }
    ...
    /**
     * Elite-guided search: combines best solution guidance with exploration
     */
    private void eliteGuidedSearch(int i, int t, double[] newPosition) {
        // Select two random individuals for differential evolution style mutation
        ...
        double F = 0.5 + 0.3 * random.nextDouble(); // Adaptive mutation factor
        double CR = 0.9; // Crossover probability

        for (int d = 0; d < dimensions; d++) {
            if (random.nextDouble() < CR) {
                // DE/best/1 mutation with Lévy flight
                double levyStep = generateLevyStep() * 0.1;
                newPosition[d] = bestSolution[d] + F * (population[r1][d] - population[r2][d]) + levyStep;
            } else {
                // Keep original
