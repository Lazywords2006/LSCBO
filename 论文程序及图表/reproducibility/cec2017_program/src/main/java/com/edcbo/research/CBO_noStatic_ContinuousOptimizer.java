...
/**
 * CBO-noStatic 优化器 (仅由于消融实验)
 * 将原始CBO里Attacking阶段的静态权重alpha=0.3改为非线性动态衰减权重
 */
public class CBO_noStatic_ContinuousOptimizer implements RecordableOptimizer {
    ...
    public CBO_noStatic_ContinuousOptimizer(BenchmarkFunction function, int dimensions, long seed) {
        ...
        for (int t = 0; t < MAX_ITERATIONS; t++) {
            ...
            for (int i = 0; i < POPULATION_SIZE; i++) {
                ...
                if (progress < 0.33) {
                    ...
                    dynamicAttacking(i, t, newPosition); // 传入t
                }
    ...
    // ⭐ 修改点: 使用动态权重替代原来的固定alpha=0.3
    private void dynamicAttacking(int i, int t, double[] newPosition) {
        ...
        for (int d = 0; d < dimensions; d++) {
            ...
            newPosition[d] = w * population[i][d] + (1 - w) * bestSolution[d] + perturbation;
        }
    }
