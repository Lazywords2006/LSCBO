...
public class CEC2017_BatchTest {
    ...
    // 算法名称 (移除HHO/AOA，替换为DE/SA)
    private static final String[] ALGORITHM_NAMES = {
            "LSCBO", "CBO", "PSO",
            "GWO", "DE", "SA", "GTO"
    };

    // 基准函数（使用functions包下的类）- CEC2017函数（F2已废止，共29个）
    private static final BenchmarkFunction[] BENCHMARK_FUNCTIONS = {
            // F1, F3: 单峰函数 (Unimodal) - 注意：F2已在CEC2017中废止
            new Sphere(), // F1
            new Zakharov(), // F3

            // F4-F10: 简单多峰函数 (Simple Multimodal)
            new Rosenbrock(), // F4
            new Rastrigin(), // F5
            new Ackley(), // F6
            new Griewank(), // F7
            ...
            // F11-F20: 混合/复杂函数 (Hybrid/Complex)
            new BentCigar(), // F11
            new Discus(), // F12
            ...
            new HappyCat(), // F15
            new Step(), // F16
            new Bohachevsky(), // F17 - 替代Powell(需要4k维度)
            new Quartic(), // F18
            new Exponential(), // F19
            new Alpine(), // F20

            // F21-F30: 组合/特殊函数 (Composition/Special)
            ...
            new Weierstrass(), // F28
            new XinSheYang(), // F29
            new SumOfDifferentPowers() // F30 - 替代SixHumpCamel(只支持2D)
    };
    ...
    private static RecordableOptimizer runAlgorithm(String algorithmName, BenchmarkFunction function, long seed) {
        switch (algorithmName) {
            ...
            case "DE":
                return new DE_ContinuousOptimizer(function, DIMENSIONS, seed);
            case "SA":
                return new SA_ContinuousOptimizer(function, DIMENSIONS, seed);
            case "GTO":
